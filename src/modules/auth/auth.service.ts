...
type DecodedTokenClaims = {
  // SPRINT-34: model only the untrusted claims used for diagnostics
...
type AppleTokenPayload = {
  // SPRINT-34: describe the verified Apple claims used for authorization
...
export class AuthService {
  ...
  constructor(
    ...
    try {
      // SPRINT-34: reuse the request-time Google configuration validator
      googleAudiences = this.getGoogleVerifyAudiences(); // SPRINT-34: count every accepted Google client ID
    } catch (error) {
      // SPRINT-34: a missing Google configuration is reported by the summary and rejected only on use
    ...
    if (
      // SPRINT-34: replace obsolete request-time Apple identifier failures with one startup warning
      ...
      this.logger.warn(
        // SPRINT-34: surface missing Apple configuration without blocking users
    ...
    this.logger.log(
      // SPRINT-34: emit one PM2-visible startup configuration summary
  ...
  private getAppleVerifyAudiences(): string[] {
    // SPRINT-34: return every Apple audience accepted by verification
    ...
    const extras = // SPRINT-34: allow opt-in development audiences without weakening production defaults
      this.configService
        .get<string>('APPLE_EXTRA_AUDIENCES') // SPRINT-34: read comma-separated development audiences
  ...
  private getGoogleVerifyAudiences(): string[] {
    const ids = [
      // SPRINT-34: collect each client type that can mint a ComLinkr Google ID token
    ...
    if (audiences.length === 0) {
      // SPRINT-34: Google has no safe hardcoded client-ID fallback
  ...
  private decodeTokenClaims(token: string): DecodedTokenClaims {
    // SPRINT-34: decode untrusted JWT claims solely for failure diagnostics
    try {
      // SPRINT-34: malformed input must still reach authoritative verification
      ...
      const decoded = JSON.parse(
        // SPRINT-34: parse the unverified payload for safe claim-only logging
      ...
      return {
        // SPRINT-34: expose only the two diagnostic claims
        aud: audience, // SPRINT-34: provide received audience to failure logging
        // SPRINT-34: normalize the untrusted issuer for a single structured log line
        iss:
      ...
      // SPRINT-34: never authorize from or fail early on unverified decoding
  ...
  private hasAppleRevocationCredentials(): boolean {
    // SPRINT-34: centralize revocation-only configuration checks
    return Boolean(
      // SPRINT-34: report whether every required server-to-server credential exists
  ...
  private getAppleServerCredentials(): {
    // SPRINT-34: resolve normalized credentials for code exchange and revocation
    ...
    // SPRINT-34: return null when revocation is not configured
    ...
    return {
      // SPRINT-34: provide complete normalized credentials
  ...
  private createAppleClientSecret(): string | null {
    // SPRINT-34: generate the short-lived ES256 secret used only with Apple server endpoints
    ...
    return appleSignin.getClientSecret({
      // SPRINT-34: delegate standards-compliant client-secret signing to the installed package
  ...
  private formatUserResponse(user: {
    ...
    return {
      ...
      avatarUrl: resolveMediaUrl(
        // SPRINT-47: only this field — resolve stored media the same way every other formatter does
  ...
  private async consumeTempToken(token: string): Promise<{
    // SPRINT-34: include optional revocation material in one-time OAuth registration data
  ...
  async googleAuth(
    ...
    let payload: {
      // SPRINT-34: model the verified Google claims used below
    ...
    try {
      // SPRINT-34: log every cryptographic or issuer verification failure with audience context
      const ticket = await this.googleClient.verifyIdToken({
        // SPRINT-34: verify signature, expiry, and all accepted audiences
      ...
      if (
        // SPRINT-34: visibly enforce both valid Google issuer forms
      ...
      payload = verifiedPayload; // SPRINT-34: retain the library-verified claims
    } catch (err) {
      // SPRINT-34: normalize and report the underlying verification error
      const error = err instanceof Error ? err : new Error(String(err)); // SPRINT-34: guarantee name and message fields for logging
      this.logger.warn(
        // SPRINT-34: produce one complete PM2-visible Google verification failure line
      ...
      throw new UnauthorizedException({
        // SPRINT-34: preserve a safe client-facing authentication failure
    ...
    if (!payload.email) {
      // SPRINT-34: require an identity email after verification
      throw new UnauthorizedException({
        // SPRINT-34: reject payloads that cannot resolve an account
    ...
    if (!emailVerified) {
      // SPRINT-34: prevent unsafe email-based auto-linking
  ...
  private async exchangeAppleAuthorizationCode(
    // SPRINT-34: exchange Apple's one-time code without ever breaking sign-in
    authorizationCode?: string, // SPRINT-34: accept the optional backward-compatible mobile field
  ): Promise<string | null> {
    // SPRINT-34: return only a usable refresh token
    ...
    if (!this.hasAppleRevocationCredentials()) {
      // SPRINT-34: keep private-key checks out of identity-token verification
      this.logger.warn(
        // SPRINT-34: explain why compliant refresh-token storage was skipped
    ...
    try {
      // SPRINT-34: isolate every Apple token-endpoint failure
      ...
      const response = (await Promise.race([
        // SPRINT-34: await storage-critical exchange only within a safe latency bound
        appleSignin.getAuthorizationToken(code, {
          // SPRINT-34: request tokens with the authorization-code grant
        ...
        new Promise<never>(
          (
            _,
            reject, // SPRINT-34: prevent a slow Apple endpoint from timing out user sign-in
          ) =>
            setTimeout(
              // SPRINT-34: reject the optional operation after the bounded wait
      ...
      }; // SPRINT-34: handle both package success and Apple error payloads
      if (response.error) {
        // SPRINT-34: the package returns Apple endpoint errors instead of throwing
      ...
      // SPRINT-34: prevent exchange failures from affecting authentication
      const error = err instanceof Error ? err : new Error(String(err)); // SPRINT-34: normalize unknown package errors
      this.logger.warn(
        // SPRINT-34: emit the required exchange diagnostic prefix
  ...
  public async revokeAppleAuthorization(userId: string): Promise<void> {
    // SPRINT-34: attempt Apple revocation before account data is purged
    try {
      // SPRINT-34: guarantee revocation never prevents deletion
      const provider = await this.prisma.authProvider.findUnique({
        // SPRINT-34: load the one Apple provider row allowed per user
      ...
      if (!provider.refreshToken) {
        // SPRINT-34: support accounts created before authorization-code collection
        this.logger.log(
          // SPRINT-34: record the expected pre-contract-change skip path
      ...
      if (!this.hasAppleRevocationCredentials()) {
        // SPRINT-34: require private credentials only in the revocation path
        this.logger.warn(
          // SPRINT-34: surface incomplete compliance configuration
      ...
      const response = (await appleSignin.revokeAuthorizationToken(
        provider.refreshToken,
        {
          // SPRINT-34: call Apple's revocation endpoint before database deletion
      ...
      if (response?.error) {
        // SPRINT-34: treat a non-throwing Apple error payload as revocation failure
      ...
      // SPRINT-34: deletion must proceed after every revocation failure
      const error = err instanceof Error ? err : new Error(String(err)); // SPRINT-34: normalize unknown package errors
      this.logger.warn(
        // SPRINT-34: emit the required non-blocking revocation diagnostic
  ...
  async appleAuth(
    // SPRINT-34: verify native Apple tokens and handle repeat sign-ins safely
    ...
    try {
      // SPRINT-34: capture library and explicit issuer failures in one structured log path
      const verified = await appleSignin.verifyIdToken(dto.idToken, {
        // SPRINT-34: verify against Apple's rotating public JWKS
      ...
      payload = verified; // SPRINT-34: use claims only after signature and audience verification
      if (payload.iss !== APPLE_ISSUER) {
        // SPRINT-34: explicitly enforce Apple's documented issuer
      ...
      this.logger.warn(
        // SPRINT-34: emit one complete PM2-visible verification failure line
      ...
      throw new UnauthorizedException({
        // SPRINT-34: preserve a generic client-facing authentication failure
    ...
    const existingProvider = await this.prisma.authProvider.findFirst({
      // SPRINT-34: perform stable-subject lookup before any email lookup
      where: {
        // SPRINT-34: constrain lookup to the Apple provider namespace
    ...
    if (existingProvider) {
      // SPRINT-34: return the already-linked account without duplicate rows
      ...
      if (!user.isActive && !pendingDeletion) {
        // SPRINT-34: reject permanently inactive accounts
        throw new UnauthorizedException({
          // SPRINT-34: avoid disclosing account state
      ...
      const refreshToken = await this.exchangeAppleAuthorizationCode(
        // SPRINT-34: opportunistically obtain revocation material
      ...
      if (refreshToken) {
        // SPRINT-34: never clear a previously stored valid refresh token
        await this.prisma.authProvider.update({
          // SPRINT-34: persist newly returned revocation material
      ...
      await this.prisma.user.update({
        // SPRINT-34: preserve existing last-active tracking without mutating name or email
    ...
    if (!email) {
      // SPRINT-34: only a previously linked subject may sign in without an email claim
    ...
    const existingUser = await this.prisma.user.findUnique({
      // SPRINT-34: perform email auto-link only after stable-subject lookup fails
    ...
    if (existingUser) {
      // SPRINT-34: preserve the existing OAuth auto-link behavior
      const provider = await this.prisma.authProvider.create({
        // SPRINT-34: link Apple's stable subject to the existing account
        data: {
          // SPRINT-34: create exactly one Apple provider row
      ...
      const refreshToken = await this.exchangeAppleAuthorizationCode(
        // SPRINT-34: obtain optional revocation material after the provider exists
        dto.authorizationCode, // SPRINT-34: use the native one-time authorization code
      ); // SPRINT-34: complete optional auto-link exchange
      if (refreshToken) {
        // SPRINT-34: update only when Apple returned a usable token
        await this.prisma.authProvider.update({
          // SPRINT-34: attach refresh token to the new provider row
    ...
    const refreshToken = await this.exchangeAppleAuthorizationCode(
      // SPRINT-34: preserve revocation material through username completion
    ...
    const tempToken = await this.createTempToken({
      // SPRINT-34: preserve the existing needs-username flow
